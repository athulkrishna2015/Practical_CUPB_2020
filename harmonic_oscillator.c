...
/*19msphcp07 OMEGA JENA on 10.04.2020*/
...
>> "sol for critically damped H.Osc is ,x(t)={x0*exp^-(b/2m)t}cosw, where w=w=sqrt((k/m)-((b*b)/(4*m))),E=exp^-(b/2m)t"
...
b_sr=b*b;
    if
    (c_4ac>b_sr)/*condition for complex roots*/
...
{   	 printf("    \nenter the value of x0,t and and n(for no of observations you want): eg: 3 10 100\n");
    ...
    {
    if
    (c_4ac<b_sr)/*condition for real roots*/
    ...
    if
   (c_4ac==b_sr)/*condition for unique and single root*/
        {
            ru=-b/2*a;
            printf("\nONLY ONE ROOT=%f\n",ru);
